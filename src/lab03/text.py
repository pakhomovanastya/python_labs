...
def top_n(freq: dict[str, int], n: int = 5) -> list[tuple[str, int]]:
    ...
    list и тд будет лист/значение
    lambda возращяет пару из двух эл., где эти пары сравнивает
    чем боьше число, то мы делаем отрицательным и оно будет раньше стоять
    '''
    return top
