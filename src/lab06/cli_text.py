...
# def cat(fpath: str , top_n: int) -> None:
...
def cat(fpath: str, numeration: bool) -> None:
    ...
    with open(fpath, "r", encoding="utf-8") as fr:
        ...
        for i, line in enumerate(fr.readlines(), 1):
            # enumerate превращает в пары (номер, элемент коллекции), начиная с 1
            # strip() удаляет лишние пробелы и переводы строк в начале и конце
            print(f"{i}: {line.strip()}" if numeration else f"{line.strip()}")


def stats(input, top):
    with open(input, "r", encoding="utf-8") as fr:
        analyze_text(fr.read(), top)
...
def main():
    ...
    cat_parser.add_argument(
        "--input", required=True, help="Относительнаый(абсолютный) путь до файла"
    )
    ...
    stats_parser.add_argument(
        "--top", type=int, default=5, help="Количество самых частых слов"
    )
    try:
        args = parser.parse_args()
