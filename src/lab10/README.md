...
>
>Односвязный список (Singly Linked List) - цепочка связанных элементов
lab10/structures.py
---
Стек - это структура данных, работающая по принципу "последний пришел - первый ушел" (LIFO). Представьте стопку тарелок:
```python
class Stack:
    def __init__(self):
...
Очередь (Queue)
---
Очередь - это структура данных, работающая по принципу "первый пришел - первый ушел" (FIFO). Представьте очередь в магазине:
...
```
lab10/linked_list.py Односвязный список Singly Linked List
---
Связный список - это цепочка элементов (узлов), где каждый элемент:
